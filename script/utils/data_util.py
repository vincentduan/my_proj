...
import pickle
from script.utils.make_edges_orign import *
...
def loader(dataset='enron10', split_count=10):
    ...
    data = load_dataset(dataset, split_count)
    torch.save(data, filepath)
    return data


def load_dataset(dataset, split_count):
    with open('../data/input/raw/{}/{}/adj_time_list.pickle'.format(dataset,split_count), 'rb') as handle:
    ...
    # 原始使用edges, 会出现enron数据集节点个数不正确的问题, 使用biedges就可以解决
    data['num_nodes'] = int(np.max(np.vstack(biedges))) + 1
